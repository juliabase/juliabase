...
=======================================================================================
  ...
  Diese Unit bietet nur eine einzige Funktion an, nämlich execute_chantal.  Diese
  ...
  anzulegen, kann man aufrufen:
      ...
      execute_chantal('b.bunny', 'bugspasswort', 'new_samples(1, u"Peters Büro")');
...
var
  package_path,python_path:String;
  open_error_page_in_browser: boolean;

function execute_chantal(const login, password, commands: String; testserver:boolean=false):String;
...
constructor EChantalError.Create(ErrorCode:integer; message_:string; testserver:boolean);
var
  url: String;
...
begin
  ...
  begin
    if testserver then url := 'http://bob.ipv.kfa-juelich.de' else url := 'http://chantal.ipv.kfa-juelich.de';
    closing_brace := Pos(')', message_);
    url := url + copy(message_, closing_brace + 2, length(message_) - closing_brace - 1);
...
function execute_chantal(const login, password, commands: String; testserver:boolean=false):String;
const
  chantal_exception_prefix = 'chantal_remote.ChantalError: ';
var
...
begin
  ...
  full_input := format('# -*- coding: utf-8 -*-'#13#10'import sys; sys.path.append("%s");from chantal_remote import *;' +
                       'login("%s", "%s", testserver=%s);%s;logout()'#26,
		       [package_path, login, password, testserver_string, commands]);
  ...
  begin
    ...
    until line_ending = 0;
    if copy(last_line, 1, length(chantal_exception_prefix)) = chantal_exception_prefix then
    begin
      delete(last_line, 1, length(chantal_exception_prefix));
      closing_brace := Pos(')', last_line);
      raise EChantalError.Create(StrToInt(copy(last_line, 2, closing_brace - 2)), last_line, testserver)
...
begin
  package_path := 'p:/Chantal/remote_client';
  python_path := 'p:/Chantal/Python2.7/python.exe';
