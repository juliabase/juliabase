...
Layouts are set by the `chantal_ipv.models.Structuring` process.  It defines
...
import chantal_ipv.models
from chantal_ipv.reportlab_config import default_fontname
...
def get_current_structuring(sample, timestamp=None):
    ...
    :rtype: `chantal_ipv.models.Structuring`

    :Exceptions:
      - `NoStructuringFound`: if no matching structuring was found
    """
    query_set = chantal_ipv.models.Structuring.objects.filter(samples=sample)
    if timestamp:
        query_set = query_set.filter(timestamp__lte=timestamp)
    try:
        return query_set.latest("timestamp")
    except chantal_ipv.models.Structuring.DoesNotExist:
...
class Layout(object):
    ...
    :type structuring: `chantal_ipv.models.Structuring`
    """
...
class CellsLayout(Layout):
    ...
    def _get_colors_and_labels(solarsimulator_measurement):
        ...
        :type solarsimulator_measurement:
          `chantal_ipv.models.SolarsimulatorMeasurement`
    ...
    def draw_layout(self, filename):
        ...
        if isinstance(self.process, (chantal_ipv.models.SolarsimulatorPhotoMeasurement,
                                     chantal_ipv.models.SolarsimulatorDarkMeasurement)):
