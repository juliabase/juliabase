...
class ProcessForm(form_utils.ProcessForm):
    ...
    def clean_number(self):
        ...
        if number:
            ...
            if ipv_models.LargeEvaporation.objects.filter(number=number).exists() and not self.edit:
                raise ValidationError(_("The evaporation number you have chosen already exists."))
        return number

    class Meta:
        model = ipv_models.LargeEvaporation
...
class FormSet(object):
    ...
    :type process: `ipv_models.LargeEvaporation` or ``NoneType``
    """
    ...
    def __init__(self, request, process_number):
        ...
        self.process = \
            get_object_or_404(ipv_models.LargeEvaporation, number=process_number) if process_number else None
    ...
    def from_database(self, query_dict):
        ...
        try:
            number = ipv_models.LargeEvaporation.objects.latest('number').number
        ...
        if not self.process and copy_from:
            ...
            source_process_query = ipv_models.LargeEvaporation.objects.filter(number=copy_from)
    ...
    def save_to_database(self):
        ...
        :rtype: `ipv_models.LargeEvaporation` or ``NoneType``
        """
...
def edit(request, process_number):
    ...
    ipv_model = ipv_models.LargeEvaporation
