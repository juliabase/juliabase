...
class DepositionForm(form_utils.ProcessForm):
    ...
    def clean_number(self):
        ...
        number = form_utils.clean_deposition_number_field(number, "O")
...
class LayerForm(forms.ModelForm):
    """Model form for a hot-wire layer in the p-hot-wire deposition."""
    ...
    def __init__(self, data=None, **kwargs):
        ...
        for fieldname in ["h2", "sih4", "mms", "tmb", "ph3_sih4", "ch4", "ar", "tmal"]:
...
class FormSet(object):
    """Class for holding all forms of the p-hot-wire deposition views, and for
    ...
    def from_database(self, query_dict):
        ...
        if not self.deposition and copy_from:
            ...
            if source_deposition_query.count() == 1:
                ...
                deposition_data["number"] = utils.get_next_deposition_number("O")
            ...
            if self.deposition:
                ...
                self.deposition_form = DepositionForm(
                    self.user, initial={"operator": self.user.pk, "timestamp": datetime.datetime.now(),
                                        "number": utils.get_next_deposition_number("O")})
                self.layer_forms, self.change_layer_forms = [], []
...
def edit(request, deposition_number):
    """Edit or create a p-hot-wire deposition.  In case of creation, starting
...
def show(request, deposition_number):
    """Show an existing p-hot-wire deposision.  You must be a p-hot-wire
