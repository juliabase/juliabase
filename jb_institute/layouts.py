...
Layouts are set by the `jb_institute.models.Structuring` process.  It defines
...
import jb_institute.models
from jb_institute.reportlab_config import default_fontname
...
def get_current_structuring(sample, timestamp=None):
    ...
    :rtype: `jb_institute.models.Structuring`

    :Exceptions:
      - `NoStructuringFound`: if no matching structuring was found
    """
    query_set = jb_institute.models.Structuring.objects.filter(samples=sample)
    if timestamp:
        query_set = query_set.filter(timestamp__lte=timestamp)
    try:
        return query_set.latest("timestamp")
    except jb_institute.models.Structuring.DoesNotExist:
...
class Layout(object):
    ...
    :type structuring: `jb_institute.models.Structuring`
    """
    width = 80 * mm
    height = 80 * mm
...
class CellsLayout(Layout):
    ...
    def _get_colors_and_labels(solarsimulator_measurement):
        ...
        :type solarsimulator_measurement:
          `jb_institute.models.SolarsimulatorMeasurement`
