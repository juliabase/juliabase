        ...
        for i, layer in enumerate(layer_forms):
            if layer.is_valid() and all([channel.is_valid() for channel in channel_forms[i]]) and \
                ...
                new_channel_lists.append(
                        [SixChamberChannelForm(prefix_dict(channel.cleaned_data, "%d_%d"%(layer_index, channel_index)),
    ...
    if request.method == "POST":
        ...
        for layer_index in layer_indices:
            channel_forms.append(
                [SixChamberChannelForm(request.POST, prefix="%d_%d"%(layer_index, channel_index))
        ...
        if all_valid and not structure_changed:
            ...
            six_chamber_deposition.sixchamberlayer_set.all().delete()  # deletes channels, too
        ...
        for layer_index, layer in enumerate(layers):
            channel_forms.append(
                [SixChamberChannelForm(prefix="%d_%d"%(layer_index, channel_index), instance=channel)
