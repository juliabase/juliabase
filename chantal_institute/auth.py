...
and permissions.  See the `chantal_institute.synchronize_with_ad` function.
...
class LDAPConnection(object):
    ...
    def __init__(self):
        self.connection = ldap.initialize(settings.AD_LDAP_URL)
    ...
    def is_valid(self, username, password):
        ...
        try:
            ...
            bound_connection = ldap.initialize(settings.AD_LDAP_URL)
    ...
    def is_eligible_ldap_member(self, username):
        """Returns whether a user really is a member of the IEK-PV, or
        another authorised member of the LDAP directory.  This method even
        works if the user cannot be found in the AD at all (it returns
        ``False`` then, of course).
        ...
        :Return:
          whether the user is in the IEF-5 alias IEK-PV, or a specially
        ...
        return attributes is not None and (
            attributes.get("department", [""])[0].upper().replace("-", "") in ["IEF5", "IEK5"]
    ...
    def get_group_names(attributes):
        ...
        :rtype: set of str
        """
        group_paths = attributes.get("memberOf", [])
    ...
    def synchronize_with_ad(self, user):
        ...
        if self.is_eligible_ldap_member(user.username):
            ...
            if "department" in attributes:
                user.chantal_user_details.department = attributes["department"][0].decode("utf-8")
