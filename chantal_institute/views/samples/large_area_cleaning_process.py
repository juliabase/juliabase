...
class CleaningProcessForm(form_utils.ProcessForm):
    ...
    class Meta:
        model = institute_models.LargeAreaCleaningProcess
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        old_cleaning_numbers = list(institute_models.LargeAreaCleaningProcess.objects.filter(cleaning_number__startswith=current_year).
    ...
    def clean_cleaning_number(self):
        ...
        if not self.old_cleaningprocess and cleaning_number:
            ...
            if institute_models.LargeAreaCleaningProcess.objects.filter(cleaning_number=cleaning_number).exists():
...
def edit(request, large_area_cleaning_process_id):
    ...
    :rtype: ``HttpResponse``
    """
    cleaning_process = get_object_or_404(institute_models.LargeAreaCleaningProcess, pk=utils.convert_id_to_int(large_area_cleaning_process_id)) \
        if large_area_cleaning_process_id else None
    permissions.assert_can_add_edit_physical_process(request.user, cleaning_process, institute_models.LargeAreaCleaningProcess)
