...
"""In most cases, only one layer is required.
Because of that, a layerform is always created at the first time.
"""
...
class DepositionForm(form_utils.ProcessForm):
    ...
    def __init__(self, user, data=None, **kwargs):
        ...
        if not user.is_staff:
            self.fields["loadlock"].choices = [choice for choice in self.fields["loadlock"].choices if choice[0] != "??"]
    ...
    def clean_number(self):
        ...
        if not deposition_number_pattern.match(number):
    ...
    def clean(self):
        _ = ugettext
        self.cleaned_data = super(DepositionForm, self).clean()
...
class LayerForm(forms.ModelForm):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        for fieldname in ["rpm", "temperature_ll", "pre_heat",
                          "operating_pressure", "base_pressure", "throttle", "gen_power",
                          "ref_power", "cl", "ct", "feed_rate", "steps", "static_time", "ar_o2", "n2", "frequency",
                          "duty_cycle", "accumulated_power"]:
            self.fields[fieldname].widget.attrs["size"] = "10"
        for fieldname in ["temperature_smc_1", "temperature_smc_2", "temperature_smc_3",
                          "temperature_pc_1", "temperature_pc_2", "temperature_pc_3",
                          "voltage_1", "voltage_2", "current_1", "current_2", "ar_1", "ar_2", "o2_1", "o2_2",
                          "pem_1", "pem_2", "u_cal_1", "u_cal_2", "calibration_1", "calibration_2"]:
            self.fields[fieldname].widget.attrs["size"] = "5"

    def clean(self):
        _ = ugettext
        self.cleaned_data = cleaned_data = super(LayerForm, self).clean()
        rdm = cleaned_data.get("rpm")
        if rdm and cleaned_data.get("temperature_smc_1") is not None:
            append_error(self, _("With RDM, only the middle T_S^MC can be given."), "temperature_smc_1")
        if rdm and cleaned_data.get("temperature_smc_3") is not None:
            append_error(self, _("With RDM, only the middle T_S^MC can be given."), "temperature_smc_3")
        if cleaned_data.get("mode") not in ["MF", "pulse", "R-MF"]:
            if cleaned_data.get("frequency") is not None:
                append_error(self, _("This field is forbidden if mode is neither “MF” nor “R-MF” nor “pulse”."),
                             "frequency")
            if cleaned_data.get("duty_cycle") is not None:
                append_error(self, _("This field is forbidden if mode is neither “MF” nor “R-MF” nor “pulse”."),
                             "duty_cycle")
        return cleaned_data
...
class FormSet(object):
    ...
    def from_post_data(self, post_data):
        ...
        self.layer_forms = [LayerForm(self.user, self.post_data, prefix=str(layer_index)) for layer_index in indices]
    ...
    def from_database(self, query_dict):
        ...
        if not self.deposition and copy_from:
            ...
            if source_deposition_query.count() == 1:
                ...
                deposition_data["number"] = utils.get_next_deposition_number("V")
            ...
            if self.deposition:
                ...
                self.deposition_form = DepositionForm(
                    self.user, initial={"operator": self.user.pk, "timestamp": datetime.datetime.now(),
                                        "number": utils.get_next_deposition_number("V")})
                self.layer_forms = []
        ...
        *number*, whereas the internal numbers used as prefixes in the HTML
        ...
        for new_layer in new_layers:
            if new_layer[0] == "original":
                ...
                next_layer_number += 1
                self.layer_forms.append(LayerForm(self.user, post_data, prefix=prefix))
                ...
                if original_layer.is_valid():
                    ...
                    next_layer_number += 1
                    self.layer_forms.append(LayerForm(self.user, initial=layer_data, prefix=str(next_prefix)))
                ...
                self.layer_forms.append(LayerForm(self.user, initial=initial, prefix=str(next_prefix)))
                self.change_layer_forms.append(ChangeLayerForm(prefix=str(next_prefix)))
                next_layer_number += 1
                next_prefix += 1
            else:
    ...
    def get_context_dict(self):
        ...
        return {"deposition": self.deposition_form,
                ...
                "add_layers": self.add_layers_form,
...
def edit(request, deposition_number):
    """Edit or create a large sputter deposition.  In case of creation, starting
...
def show(request, deposition_number):
    """Show an existing large_sputter_deposision.  You must be a large sputter
