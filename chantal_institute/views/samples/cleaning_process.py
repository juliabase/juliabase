...
"""Views to add and edit cleaning processes.
"""
...
class CleaningProcessForm(form_utils.ProcessForm):
    ...
    def clean(self):
        ...
        if "cleaning_number" in cleaned_data and "timestamp" in cleaned_data:
            if int(cleaned_data["cleaning_number"][:2]) != cleaned_data["timestamp"].year % 100:
...
def edit(request, cleaning_process_id):
    """Central view for editing and creating cleaning processes.  If
    ``cleaning_process_id`` is ``None``, a new cleaning process is created.
    ...
    :type cleaning_process_id: unicode or ``NoneType``
        ...
        if all([cleaning_process_form.is_valid(), samples_form.is_valid() or not samples_form.is_bound,
            ...
            if cleaning_process:
                return utils.successful_response(
                    request, _("Cleaning process {name} was successfully changed in the database."). \
                        format(name=new_cleaning_process),
                    new_cleaning_process.get_absolute_url())
            else:
                return utils.successful_response(
                    request, _("Cleaning process {name} was successfully added to the database."). \
    ...
    title = _("Edit cleaning process “{name}”").format(name=cleaning_process) if cleaning_process \
        else _("Add cleaning process")
