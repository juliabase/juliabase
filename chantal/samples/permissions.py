...
class PermissionError(Exception):
    ...
    :ivar cause: the full cause why the permission was missing.  It should be a
      complete sentence, starting with a capital letter and ending with a full
      stop.  It should address the user directly.  For example, it may contain
      ``"You are not in group “SiC”."``.  Avoid giving more information here
      than the user is allowed to know.  Typically, this is generated by the
      assertion function.

    :type action: unicode
    :type cause: unicode
    """
    def __init__(self, user, action, cause):
        super(PermissionError, self).__init__(_(u"Permission missing for: %(action)s  Cause: %(cause)s") % (action, cause))
        self.user, self.action, self.cause = user, action, cause

class PermissionViewSampleError(PermissionError):
    def __init__(self, user, sample, cause):
        super(PermissionViewSampleError, self).__init__(user, _(u"Viewing sample %s.") % sample, cause)
        self.sample = sample
...
def assert_can_view_sample(user, sample):
    if not user.has_perm("samples.can_view_all_samples") and sample.group not in user.groups.all() \
        ...
        raise PermissionViewSampleError(user, sample, _(u"You are not in the sample's group, nor are you its currently "
                                                        u"responsible person, nor are you a senior member."))
