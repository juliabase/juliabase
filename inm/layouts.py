...
Layouts are set by the `inm.models.Structuring` process.  It defines the
...
import inm.models
from inm.reportlab_config import default_fontname
...
def get_current_structuring(sample, timestamp=None):
    ...
    except inm.models.Structuring.DoesNotExist:
...
class CellsLayout(Layout):
    ...
    def draw_layout(self, canvas):
        if isinstance(self.process, (inm.models.SolarsimulatorMeasurement)):
