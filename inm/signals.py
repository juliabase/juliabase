...
from inm import models as inm_app
...
def add_sample_details(sender, instance, created, **kwargs):
    ...
    """
    if created:
        inm_app.SampleDetails.objects.get_or_create(sample=instance)
            ...
            for process in sample.processes.all():
                ...
                if hasattr(process, "update_informal_layers"):
                    ...
                    process.update_informal_layers(
                        ...
                        lambda **kwargs: inm_app.InformalLayer(sample_details=sample.sample_details,
