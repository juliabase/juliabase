...
shapes_juelich = {"1": ((18, 18.5), (10, 10)),
                  ...
                  "35": ((58, 82.5), (20, 5)),
                  "36": ((81.5, 82.5), (10, 5))}
areas_juelich = {position: shapes_juelich[1][0] * shapes_juelich[1][1] / 100 for position, shapes_juelich in shapes_juelich.items()}
...
U/V{column_headers}"""
    header_data = {"timestamp": datetime.datetime(2014, 11, 8, 10, measurement_index, 0).strftime("%Y-%m-%d %H:%M:%S"),
                   ...
                   "areas": " ".join(str(areas_juelich[position]) for position in measured_positions),
                   "column_headers": len(measured_positions) * "  I/A"}
    data = [voltages]
    for position in measured_positions:
        area = areas_juelich[position]
